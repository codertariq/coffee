...
var DatatableButtonsHtml5 = function() {
    ...
    var _componentDatePicker = function() {
        ...
        // console.log(local);
        $('#dob, #nomenee_dob, #joining_date').daterangepicker({
    ...
    var _componentCheckMember = function() {
        $(document).on('change', '#refarence_member_id', function() {
            $('.refarance_member').remove();
           var url = $(this).data('url');
           $.ajax({
                    url: url+'/?member_id='+$(this).val(),
                    type: 'Get',
                    dataType: 'JSON'
                })
                .done(function(data) {
                    $('#refarence_member_id').after('<div class="refarance_member text-'+data.status+'">' + data.message + '</div');
                })
                .fail(function(data) {
                    console.log(data);
                });
