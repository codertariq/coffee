...
var DatatableButtonsHtml5 = function() {
    ...
    var _componentDatatableButtonsHtml5 = function() {
        ...
        tariq = $('.content_managment_table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: Base_url+'settings/payment/datatable',
            columns: [{
                data: 'id',
                name: 'id'
            }, {
    ...
    var _componentChangeStatus = function() {
     ...
     var _componentGetFormFields = function() {
        $(document).on('click', '#add_field', function() {
            ...
            //console.log(type);
            $.ajax({
                    url: '/admin/settings/payment/fields?type=' + type + '&row=' + row,
                    type: 'Get',
                    dataType: 'html'
                })
                .done(function(data) {
                ...
                .fail(function(data) {
                    //console.log(data);
                    $('#invoice_item').append('<span class="text-danger text-center ajaxerror"> Something Went Wrong. Please Try again later.......</span>');
                    $('#ajaxloader').hide();
                });
        });
    }
    var _componentDeleteFields = function() {
            $(document).on('click', '#delete_row', function() {
                var row = $(this).data('id');
                //console.log('#row_id_' + row)
                var total_row = parseInt($('#total_row').val());
                $('#row_id_' + row).remove();
                $('#total_row').val(total_row - 1);
            })
    ...
    return {
        init: function() {
            ...
            _componentGetFormFields();
            _componentDeleteFields();
            _componentSelect2();
            _componentDatatable();
