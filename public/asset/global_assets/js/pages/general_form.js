...
var DatatableButtonsHtml5 = function() {
    ...
    var _componentGetLoanField = function() {
        $(document).on('change', '#member_id', function() {
            var member_id = $(this).val(); // it will get action url
            //console.log(json_encode(method));
            $.ajax({
                    url: Base_url + 'general/loan',
                    type: 'Post',
                    data: {
                        member_id: member_id,
                        name: 'loan_id'
    ...
    var _componentGetLoanDetails = function() {
        $(document).on('change', '#loan_id', function() {
            $('#loan_loader').show();
            var member_id = $('#member_id').val();
            var loan_id = $(this).val();
            $.ajax({
                    url: Base_url + 'general/loan_details',
                    type: 'Post',
                    data: {
                        member_id: member_id,
                        loan_id: loan_id
                ...
                .done(function(data) {
                    //console.log(data);
                    getLoanDetailInfo(member_id, loan_id);
    ...
    return {
        init: function() {
            ...
            _componentGetLoanField();
            _componentGetLoanDetails();
        }
    }
}();
...
//change cost
$(document).on('keyup', '#savings_ammount', function() {
    refreshfield();
});

$(document).on('change', '#loan_id', function() {
    $('#paid_installment').attr('readonly', false);
    refreshfield();
});

$(document).on('keyup', '#paid_installment', function() {
    var install = parseFloat($(this).val());
    var repay_ammount = parseFloat($('#repay_ammount_dt').val());
    var repay_interest = parseFloat($('#repay_interest_dt').val());
    
    var new_repay_ammount = roundToTwo(repay_ammount * install);
    var new_repay_interest = roundToTwo(repay_interest * install);
    if (isNaN(new_repay_ammount)) {
        new_repay_ammount = repay_ammount;
    }
    if (isNaN(new_repay_interest)) {
        new_repay_interest = repay_interest;
    }
    $('#repay_ammount').val(new_repay_ammount);
    $('#repay_interest').val(new_repay_interest);
    
    refreshfield();
});

$(document).on('change', '#paid_installment', function() {
    var install = parseFloat($(this).val());
    var repay_ammount = parseFloat($('#repay_ammount_dt').val());
    var repay_interest = parseFloat($('#repay_interest_dt').val());
    if (isNaN(install)) {
    ...
    $('#repay_ammount').val(roundToTwo(repay_ammount * install));
    $('#repay_interest').val(roundToTwo(repay_interest * install));
...
function calculateTotalRepayWithSavings() {
    var repay_ammount = parseFloat($('#repay_ammount').val());
    var repay_interest = parseFloat($('#repay_interest').val());
    var savings_ammount = parseFloat($('#savings_ammount').val());
    if (isNaN(repay_ammount)) {
        repay_ammount = 0;
    }
    if (isNaN(savings_ammount)) {
        savings_ammount = 0;
    }
    if (isNaN(repay_interest)) {
        repay_interest = 0;
    }
    var total_repay_with_savings = repay_ammount + repay_interest + savings_ammount;
    if (isNaN(total_repay_with_savings)) {
        total_repay_with_savings = 0;
    }
    return roundToTwo(total_repay_with_savings);
    //return total;
}
function calculateTotalSavings() {
    var savings_ammount = parseFloat($('#savings_ammount').val());
    var previous_savings = parseFloat($('#previous_savings').val());
    if (isNaN(savings_ammount)) {
        savings_ammount = 0;
    }
    if (isNaN(previous_savings)) {
        previous_savings = 0;
    }
    var total_savings = savings_ammount + previous_savings;
    if (isNaN(total_savings)) {
        total_savings = 0;
    }
    return roundToTwo(total_savings);
    //return total;
}
...
function refreshfield() {
    ...
    $('#total_savings').val(calculateTotalSavings());
    $('#total_repay_with_savings').val(calculateTotalRepayWithSavings());
}

function getLoanDetailInfo(member_id, loan_id) {
    $.ajax({
            url: Base_url + 'general/loan_details_info',
            type: 'Post',
            data: {
                member_id: member_id,
                loan_id: loan_id
        ...
        .done(function(data) {
           $('#repay_ammount').val(data.loan.loan_installment);
            $('#repay_ammount_dt').val(data.loan.loan_installment);
            $('#repay_interest').val(data.loan.interest_installment);
            $('#repay_interest_dt').val(data.loan.interest_installment);
            ...
            $('#previous_savings').val(data.current_savings);
            $('#total_savings').val(data.current_savings);
            refreshfield();
        })
        .fail(function(data) {
            console.log(data);
