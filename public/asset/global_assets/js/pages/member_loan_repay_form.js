...
var DatatableButtonsHtml5 = function() {
    ...
    var _componentGetLoanField = function() {
        $(document).on('change', '#member_id', function() {
            ...
            //console.log(json_encode(method));
            $.ajax({
                    url: Base_url + 'repay/loan/member/loan',
                    type: 'Post',
                    data: {
                        member_id: member_id,
                        name: 'loan_id'
    ...
    var _componentGetLoanDetails = function() {
        $(document).on('change', '#loan_id', function() {
            ...
            $.ajax({
                    url: Base_url + 'repay/loan/member/loan_details',
                    type: 'Post',
                    data: {
                        member_id: member_id,
                        loan_id: loan_id
...
function calculateCurrentDue() {
   var total_due = parseFloat($('#total_due').val());
    ...
    var total_current_due = roundToTwo(total_due - total_repay);
    if (isNaN(total_current_due)) {
        total_current_due = 0;
    }

    return total_current_due;
    //return total;
}
...
function getLoanDetailInfo(member_id, loan_id) {
    $.ajax({
            url: Base_url + 'repay/loan/member/loan_details_info',
            type: 'Post',
            data: {
                member_id: member_id,
                loan_id: loan_id
